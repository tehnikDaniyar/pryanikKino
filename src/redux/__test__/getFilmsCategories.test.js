import filmsInfoSlice from "../slices/filmsInfoSlice";
import { getFilmsCategories } from "../slices/filmsInfoSlice";
import { test, describe, vi, expect } from "vitest";

describe('test getFilmsCategories', async () => {

	test('getFilmsCategories calls', async () => {
		...
		console.log(start[0].type, end[0].type);

		expect(calls).toHaveLength(2);
		expect(start[0].type).toBe('filmsInfo/getFilmsCategories/pending');
		expect(end[0].type).toBe('filmsInfo/getFilmsCategories/fulfilled');
		expect(end[0].payload).toEqual(categoriesInfo);
	});
});